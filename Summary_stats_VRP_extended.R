...
get_tbill_data <- function(name) {
  ...
  data_type <- data_type %>%
    ...
    filter(month >= as.Date("1990-01-01") & month <= as.Date("2023-12-31"))
}
...
getSymbols("^SP500TR", src = "yahoo", from = as.Date("1990-01-01"), to = as.Date("2024-01-01"))
...
##########SUMMARIZE THE VRP VARIABLES UNTIL 2023-12##########
summary_variable(IV)
summary_variable(RV)
summary_variable(VRP)

table_1 <- bind_rows(
  table_1,
  summary_variable(IV[1:408]) %>% mutate(name = "IV") %>% select(name, everything())
)
table_1 <- bind_rows(
  table_1,
  summary_variable(RV[1:408]) %>% mutate(name = "RV") %>% select(name, everything())
)
table_1 <- bind_rows(
  table_1,
  summary_variable(VRP[1:408]) %>% mutate(name = "VRP") %>% select(name, everything())
)

VRP_IVminRV <- as.numeric(IV) - as.numeric(RV)
summary_variable(VRP_IVminRV)
...
table_1 <- bind_rows(
  table_1,
  summary_variable(pe_ratios$log_pe[1:408]) %>% mutate(name = "pe_ratio") %>% select(name, everything())
)
...
table_1 <- bind_rows(
  table_1,
  summary_variable(pd_ratios$log_pd[1:408]) %>% mutate(name = "pd_ratio") %>% select(name, everything())
)

############SUMMARIZE DFSP until 2024##############
...
table_1 <- bind_rows(
  table_1,
  summary_variable(tmsp$tmsp[1:408]) %>% mutate(name = "tmsp") %>% select(name, everything())
)

summary_variable(tmsp$tmsp)

############SUMMARIZE RREL until 2024##############
# From DTB1 (pre-2001)
...
table_1 <- bind_rows(
  table_1,
  summary_variable(rrel_combined$rrel[1:408]) %>% mutate(name = "rrel") %>% select(name, everything())
)
...
table_1 <- bind_rows(
  table_1,
  summary_variable(cay_monthly$cay_std[1:408]) %>% mutate(name = "cay") %>% select(name, everything())
)



regression_data <- data.frame(
  excess_return = returns_data$excess_return[2:408],
  VRP = VRP[1:407]
) 

write.xlsx(regression_data, 'regression_data_recent.xlsx')


full_data <- data.frame(
  Excess = returns_data$excess_return[2:408],
  VRP = VRP[1:407],
  IV = IV[1:407],
  RV = RV[1:407],
  PE = pe_ratios$log_pe[1:407],
  PD = pd_ratios$log_pd[1:407],
  DFSP = dfsp$dfsp[1:407],
  TMSP = tmsp$tmsp[1:407],
  RREL = rrel_combined$rrel[1:407],
  CAY = cay_monthly$cay_std[1:407]
)

full_data[] <- lapply(full_data, function(x) as.numeric(as.character(x)))

write.xlsx(full_data, 'full_data_recent.xlsx')
...
cor_data <- data.frame(
  ...
  IV = IV[2:408],
  RV = RV[2:408],
  PE = pe_ratios$log_pe[2:408],
  PD = pd_ratios$log_pd[2:408],
  DFSP = dfsp$dfsp[2:408],
  TMSP = tmsp$tmsp[2:408],
  RREL = rrel_combined$rrel[2:408],
  CAY = cay_monthly$cay_std[2:408]
)
...
cor_CAY_data <- data.frame(
  Excess = returns_data$excess_return[2:360],
  VRP = VRP[2:360],
  IV = IV[2:360],
  RV = RV[2:360],
  PE = pe_ratios$log_pe[2:360],
  PD = pd_ratios$log_pd[2:360],
  DFSP = dfsp$dfsp[2:360],
  TMSP = tmsp$tmsp[2:360],
  RREL = rrel_combined$rrel[2:360],
  CAY = cay_monthly$cay_std[2:360]
)

cor_CAY_data[] <- lapply(cor_CAY_data, function(x) as.numeric(as.character(x)))

correlation_CAY_matrix <- cor(cor_CAY_data)
